...
class Registering {
  private:
  public:
    static std::vector<marto::autoregister::transitions::TransitionFactory *>
        *transitionFactories;
...
#define class_transition(name, ns)                                             \
    ...
    int marto::autoregister::transitions::instances::name::dummy =             \
        marto::autoregister::transitions::Registering::autoregister(           \
            new marto::autoregister::transitions::TransitionFactoryImpl<       \
                ns::name>(#name));                                             \
    class ns::name : public marto::Transition

#define class_std_transition(name) class_transition(name, marto::stdlib)
